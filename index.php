<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<!-- 1, Meta Tag Responsive -->
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<!-- 2. Bootstrap CSS -->
	<link rel="stylesheet" type="text/css" href="bootstrap-4/css/bootstrap.css">
	<link rel="stylesheet" type="text/css" href="css/style.css">

	<link rel="stylesheet" type="text/css" href="assets/fontawesome/css/all.css">

	<title>TK Al-Bina | Beranda</title>
	<link rel="shortcut icon" href="assets/pic3.jpg">
</head>
<body id="my-top">

	<div id="wrapper">
		<div id="header">
			<div class="row">
				<div class="col-md-12">
					<div class="container">
						<nav class="navbar navbar-expand-sm navbar-light bg-light fixed-top" id="nav-shrink">
							<div class="container">
								<a class="navbar-brand" href="index.html"><img src="assets/logo.png"></a>
								<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
									<span class="navbar-toggler-icon"></span>
								</button>
								<div class="collapse navbar-collapse" id="navbarNav">
									<ul class="navbar-nav mr-auto">
										<li class="nav-item">
										<a class="nav-link activeku" href="index.php">Beranda</a>
										</li>
										<li class="nav-item">
										<a class="nav-link classku" href="pendaftaran.php">Pendaftaran</a>
										</li>
										<li class="nav-item">
										<a class="nav-link classku" href="profil.php" tabindex="-1" aria-disabled="true">Profil</a>
										</li>
										<li class="nav-item">
										<a class="nav-link classku" href="berita.php" tabindex="-1" aria-disabled="true">Berita</a>
										</li>
										<li class="nav-item">
										<a class="nav-link classku" href="pengumuman.php" tabindex="-1" aria-disabled="true">Pengumuman</a>
										</li>
									</ul>
								</div>
							</div>
						</nav>
					</div>
				</div>
			</div>
		</div><!-- end of header -->
		<div id="ctn-jumbotron">
			<div id="carouselExampleCaptions" class="carousel slide" data-ride="carousel">
			  <ol class="carousel-indicators">
			    <li data-target="#carouselExampleCaptions" data-slide-to="0" class="active"></li>
			    <li data-target="#carouselExampleCaptions" data-slide-to="1"></li>
			    <li data-target="#carouselExampleCaptions" data-slide-to="2"></li>
			  </ol>
			  <div class="carousel-inner">
			    <div class="carousel-item active">
			      <img src="assets/pic4.jpg" class="d-block w-100 img-height" alt="...">
			      <div class="carousel-caption d-none d-lg-block">
			        <div class="jumbotron jumbotron-fluid">
					  <div class="container">
					    <h3 class="display-5">Selamat Datang di TK Al-Bina</h3>
						<hr class="my-5" style="background-color: white;">
						<p class="lead">This is a simple hero unit, a simple jumbotron-style component for calling extra attention to featured content or information.</p>
					  </div>
					</div>
			      </div>
			    </div>
			    <div class="carousel-item">
			      <img src="assets/pic1.jpg" class="d-block w-100 img-height" alt="...">
			      <div class="carousel-caption d-none d-md-block">
			        <div class="jumbotron jumbotron-fluid">
						<h4 class="display-5">Pendaftaran Online TK Al-Bina</h4>
						<br>
					  	<a class="btn btn-primary btn-lg" href="#" role="button">Learn more</a>
					</div>
			      </div>
			    </div>
			    <div class="carousel-item">
			      <img src="assets/pic2.jpg" class="d-block w-100 img-height" alt="...">
			      <div class="carousel-caption d-none d-md-block">
			        <div class="jumbotron jumbotron-fluid">
						<h4 class="display-5">Staff & Guru Pengajar</h4>
						<br>
						<p class="lead">This is a simple hero unit, a simple jumbotron-style component for calling extra attention to featured content or information.</p>
					  </div>
			      </div>
			    </div>
			  </div>
			  <a class="carousel-control-prev" href="#carouselExampleCaptions" role="button" data-slide="prev">
			    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
			    <span class="sr-only">Previous</span>
			  </a>
			  <a class="carousel-control-next" href="#carouselExampleCaptions" role="button" data-slide="next">
			    <span class="carousel-control-next-icon" aria-hidden="true"></span>
			    <span class="sr-only">Next</span>
			  </a>
			</div>
		</div><!-- end of ctn-jumbotron -->
		<div id="ctn-vimi">
			<div class="container">
				<div class="row">
					<div class="col-lg-12 w-vimi">
						<div class="h-vimi" data-120-top="opacity:1;top:0px;" data-center-center="opacity:0;top:-200px;">
							<h1 class="vimi">Visi & Misi</h1>
						</div>
						<hr style="border: 3px solid rgb(0, 0, 0, 0.4);">
						<div class="visi">
							<h3 data-center-center="opacity:1;right:0;" data-0-bottom="opacity:0;right:-500px;">Visi</h3>
							<p data-30p-center="opacity:1;left:0;" data-30p-bottom="opacity:0;left:-200px;">Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta commodi doloremque minima corrupti optio, error deleniti quis repellendus nulla obcaecati ad! Illo itaque quo dicta odit officiis asperiores excepturi perferendis? Lorem, ipsum dolor sit amet consectetur adipisicing elit. In perferendis perspiciatis illum repellat omnis. Provident fugiat reiciendis, numquam rerum molestiae nobis temporibus, sit, odio corporis eos quia enim fugit cumque!</p>
						</div>
						<div class="misi">
							<h3 data-20p-center="opacity:1;right:0;" data-20p-bottom="opacity:0;right:-500px;">Misi</h3>
							<p data-40p-center="opacity:1;left:0;" data-40p-bottom="opacity:0;left:-200px;">Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta commodi doloremque minima corrupti optio, error deleniti quis repellendus nulla obcaecati ad! Illo itaque quo dicta odit officiis asperiores excepturi perferendis? Lorem, ipsum dolor sit amet consectetur adipisicing elit. In perferendis perspiciatis illum repellat omnis. Provident fugiat reiciendis, numquam rerum molestiae nobis temporibus, sit, odio corporis eos quia enim fugit cumque!</p>
						</div>
					</div>
				</div>
			</div>
		</div><!-- end of ctn-vimi -->
		<div id="ctn-profile">
			<div class="container" style="padding: 50px 0px !important;">
				<div class="row">
					<div class="col-md-3">
						<div class="box" data-20p-center="opacity:1;left:0;" data-20p-bottom="opacity:0;left:-300px;">
							<div class="icon">
								<i class="fas fa-trophy"></i>
							</div>
							<h3 class="title">Prestasi</h3>
							<hr>
							<p class="caption">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugiat nesciunt quidem odit dolores eligendi cumque porro.
							</p>
						</div>
					</div>
					<div class="col-md-3">
						<div class="box" data-20p-center="opacity:1;top:0;" data-20p-bottom="opacity:0;top:-300px;">
							<div class="icon">
								<i class="fas fa-building"></i>
							</div>
							<h3 class="title">Fasilitas</h3>
							<hr>
							<p class="caption">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugiat nesciunt quidem odit dolores eligendi cumque porro.
							</p>
						</div>
					</div>
					<div class="col-md-3">
						<div class="box" data-20p-center="opacity:1;bottom:0;" data-20p-bottom="opacity:0;bottom:-300px;">
							<div class="icon">
								<i class="fas fa-university"></i>
							</div>
							<h3 class="title">Kurikulum</h3>
							<hr>
							<p class="caption">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugiat nesciunt quidem odit dolores eligendi cumque porro.
							</p>
						</div>
					</div>
					<div class="col-md-3">
						<div class="box" data-20p-center="opacity:1;right:0;" data-20p-bottom="opacity:0;right:-300px;">
							<div class="icon">
								<i class="fas fa-users"></i>
							</div>
							<h3 class="title">Pengajar</h3>
							<hr>
							<p class="caption">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugiat nesciunt quidem odit dolores eligendi cumque porro.
							</p>
						</div>
					</div>
				</div>
			</div>
		</div><!-- end of ctn-profile -->
		<div id="ctn-para">
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="w-para">
							<h3>Daftar Sekarang!</h3>
						</div>
					</div>
				</div>
			</div>
		</div><!-- end of ctn-parallax -->
		<div id="ctn-latest">
			<div class="container blog">
				<h3 class="blog-judul" data-20p-center="opacity:1;top:0;" data-20p-bottom="opacity:0;top:-200px;">Latest News</h3>
				<div class="row w-blog">
					<div class="col-md-4 box-blog">
						<div class="box-post" data-20p-center="opacity:1;left:0;" data-20p-bottom="opacity:0;left:-200px;">
							<a href="#"><img src="assets/pic3.jpg"></a>
							<div class="post-desc">
								<h4>
									<a href="#">blalaa</a>
								</h4>
								<span>Mei 6, 2020</span>
								<span>
									<a class="cat" href="#">News</a>
								</span>
							</div>
						</div>
					</div>
					<div class="col-md-4 box-blog">
						<div class="box-post" data-20p-center="opacity:1;bottom:0;" data-20p-bottom="opacity:0;bottom:-200px;">
							<a href="#"><img src="assets/pic3.jpg"></a>
							<div class="post-desc">
								<h4>
									<a href="#">blalaa</a>
								</h4>
								<span>Mei 6, 2020</span>
								<span>
									<a class="cat" href="#">News</a>
								</span>
							</div>
						</div>
					</div>
					<div class="col-md-4 box-blog">
						<div class="box-post" data-20p-center="opacity:1;right:0;" data-20p-bottom="opacity:0;right:-200px;">
							<a href="#"><img src="assets/pic3.jpg"></a>
							<div class="post-desc">
								<h4>
									<a href="#">blalaa</a>
								</h4>
								<span>Mei 6, 2020</span>
								<span>
									<a class="cat" href="#">News</a>
								</span>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div><!-- end of ctn-latest -->
		<section id="footer-widget">
			<div class="container">
				<div class="row">
					<div class="col-md-6">
						<div class="fot-map">
							<h3 data-20p-center="opacity:1;left:0;" data-20p-bottom="opacity:0;left:-200px;">Our Address</h3>
							<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3966.554905196887!2d107.01441451415458!3d-6.190262295518581!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e6989678920806d%3A0xd592d9bd1fd67d77!2sTK.%20Albina!5e0!3m2!1sid!2sid!4v1588288499726!5m2!1sid!2sid" width="100%" height="350px" frameborder="0" style="border:0; margin-bottom:50px;" allowfullscreen="" tabindex="0" data-20p-center="opacity:1;top:0;" data-20p-bottom="opacity:0;top:-200px;"></iframe>
						</div>
					</div>
					<div class="col-md-4">
						<div class="fc-left" data-20p-center="opacity:1;left:0;" data-20p-bottom="opacity:0;left:-200px;">
							<h5>Our Contacts</h5>
							<p class="map">Alamat : Jl. Ujung harapan Assalam No.1
								<br> 
								Kel. Bahagia, Kec. Babelan. Kab. Bekasi</p>
							<p class="map">
								Telepon : 
								<i class="fas fa-phone" style="font-size: 16px;
								color: #ffd541;"></i>
								<a class="hp" href="tel:#">+62 821-2464-7861</a>
								<br>
								WhatsApp :
								<i class="fas fa-phone" style="font-size: 16px;
								color: #ffd541;"></i>
								<a class="hp" href="tel:#">+62 821-2464-7861</a>
								<br>
								<br>
								Jam Operasional :
								<br>
								Hari Senin s.d. Jum'at
								<br>
								Pukul 08.00 WIB - 02.00 WIB
							</p> 
						</div>
					</div>
					<div class="col-md-2">
						<div class="fc-right" data-20p-center="opacity:1;right:0;" data-20p-bottom="opacity:0;right:-200px;">
							<h5>Follow Us</h5>
							<div class="sosmed">
								<ul>
									<li>
										<a href="#">
											<i class="fab fa-facebook"></i>
										</a>
									</li>
									<li>
										<a href="#">
											<i class="fab fa-twitter"></i>
										</a>
									</li>
									<li>
										<a href="#">
											<i class="fab fa-instagram"></i>
										</a>
									</li>
									<li>
										<a href="#">
											<i class="fab fa-linkedin"></i>
										</a>
									</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section><!-- end of footer-widget -->
		<footer id="footer-end">
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<p class="text-center">Hak Cipta &copy; 2020 | Oleh : TK Al-Bina Bekasi</p>
					</div>
				</div>
			</div>
		</footer>
	</div>

	<!-- 3. jQuery Library -->
	<script type="text/javascript" src="js/jquery-3.4.1.min.js"></script>
	<!-- 4, popper.js -->
	<script type="text/javascript" src="js/poper.min.js"></script>
	<!-- 5. Bootstrap JS -->
	<script type="text/javascript" src="bootstrap-4/js/bootstrap.js"></script>
	<!-- skrollr js -->
	<script src="js/skrollr .js"></script>
	<script type="text/javascript">
		var s = skrollr.init();
	</script>

	<script>
		window.onscroll = function() {scrollFunction()};
		
		function scrollFunction() {
		  if (document.body.scrollTop > 15 || document.documentElement.scrollTop > 15) {
			document.getElementById("nav-shrink").style.padding = "5px 0px";
		  } else {
			document.getElementById("nav-shrink").style.padding = "15px 0px";
		  }
		}
	</script>
</body>
</html>